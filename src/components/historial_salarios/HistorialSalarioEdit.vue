<template>
    <div class="container mt-4">
      <h1 class="mb-4">Editar Registro de Salario</h1>
  ...
  export default {
    ...
    methods: {
      ...
      fetchHistorialSalario() {
        axios.get(`/historial_salarios/${this.$route.params.id}`)
          .then(response => {
            this.form = response.data;
          })
          .catch(error => {
            console.error("There was an error fetching the historial de salario!", error);
          });
      },
      submitForm() {
        axios.put(`/historial_salarios/${this.$route.params.id}`, this.form)
          .then(() => {
            this.$router.push({ name: 'historial_salarios.index' });
          })
          .catch(error => {
            console.error("There was an error updating the historial de salario!", error);
          });
      },
      cancel() {
        this.$router.push({ name: 'historial_salarios.index' });
